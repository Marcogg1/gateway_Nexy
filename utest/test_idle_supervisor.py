...
class TestIdleSupervisor(unittest.IsolatedAsyncioTestCase):
    """Tests for IdleSupervisor.on_param_changes."""

    def setUp(self):
        self.supervisor = IdleSupervisor()

    async def test_logs_single_change_at_info(self):
        with self.assertLogs("liftApi.idle_supervisor", level="INFO") as cm:
            await self.supervisor.on_param_changes([ParamChange(22, 1234)])
        self.assertEqual(len(cm.records), 1)
        self.assertIn("22", cm.output[0])
        self.assertIn("1234", cm.output[0])

    async def test_logs_multiple_changes_in_order(self):
        changes = [
            ParamChange(22, 100),
            ParamChange(23, 200),
            ParamChange(24, 300),
        ...
        with self.assertLogs("liftApi.idle_supervisor", level="INFO") as cm:
            await self.supervisor.on_param_changes(changes)
        self.assertEqual(len(cm.records), 3)
        self.assertIn("22", cm.output[0])
        self.assertIn("23", cm.output[1])
        self.assertIn("24", cm.output[2])

    async def test_empty_list_emits_no_logs(self):
        with self.assertNoLogs("liftApi.idle_supervisor", level="INFO"):
            await self.supervisor.on_param_changes([])

    async def test_logs_none_value(self):
        with self.assertLogs("liftApi.idle_supervisor", level="INFO") as cm:
            await self.supervisor.on_param_changes([ParamChange(22, None)])
        self.assertEqual(len(cm.records), 1)
        self.assertIn("None", cm.output[0])

    async def test_log_level_is_info_not_warning(self):
        import logging
